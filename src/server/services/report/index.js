...
const list = async (page = 1) => {
  ...
  const reports = await Report.find().sort('-createdAt').limit(Number(pageNumberOfReports)).skip(Number(pageNumberOfReports * (page - 1)))
  const reportsHit = await Report.countDocuments()
